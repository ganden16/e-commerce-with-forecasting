...
def all_forecast(request):
    if request.method == 'POST':
        try:
            ...
            results.append({'model': 'arima', 'forecast': forecastArima})

            # Simple Exponential Smoothing
            forecastSes = simple_exponential_smoothing_forecast(sales_data, steps)
            results.append({'model': 'simple-exponential-smoothing', 'forecast': forecastSes})

            # Single Moving Average
            forecastSma = single_moving_average_forecast(sales_data, steps)
            results.append({'model': 'single-moving-average', 'forecast': forecastSma})

            # Double Moving Average
            forecastDma = double_moving_average_forecast(sales_data, steps)
            results.append({'model': 'double-moving-average', 'forecast': forecastDma})

            # Weighted Moving Average
            forecastWma = weighted_moving_average_forecast(sales_data, steps)
            results.append({'model': 'weighted-moving-average', 'forecast': forecastWma})

            # LSTM Forecast
            forecastLstm = lstm_forecast(sales_data, steps)
            results.append({'model': 'long-short-term-memory', 'forecast': forecastLstm})
            
				# Auto Regressive
            forecastAr = auto_regressive_forecast(sales_data, steps)
            results.append({'model': 'auto-regressive', 'forecast': forecastAr})

            return JsonResponse({'result': results}, safe=False, status=200)
...
FORECASTING_METHODS = {
    ...
    3: "single_moving_average_forecast",
    4: "double_moving_average_forecast",
    5: "weighted_moving_average_forecast",
    6: "lstm_forecast",
    7: "auto_regressive_forecast",
}
...
def single_moving_average_forecast(sales_data, steps):
    ...
    for _ in range(steps):
        if len(current_data) < window_size:
            sma = sum(current_data) / len(current_data)
        else:
...
def double_moving_average_forecast(sales_data, steps):
    ...
    for _ in range(steps):
        ...
        if pd.isna(dma):
            dma = current_data[-1]  # fallback jika nilai NaN
        
        forecast.append(dma)
        current_data.append(dma)
...
def lstm_forecast(sales_data, steps=1):
    ...
    for _ in range(steps):
        ...
        last_sequence = next_forecast.reshape(1, 1, 1)
...
def auto_regressive_forecast(sales_data, steps=1):
    ...
    for _ in range(steps):
        ...
        model_fit = model.fit()
        next_forecast = model_fit.predict(start=len(current_data), end=len(current_data))[0]
...
def modelling(request):
    if request.method == 'POST':
        try:
            ...
            ar_fit = ar_model.fit()
            ar_forecast = ar_fit.predict(start=len(train_y), end=len(train_y) + len(test_y) - 1)
